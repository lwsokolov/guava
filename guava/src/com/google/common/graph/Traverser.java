...
public abstract class Traverser<N> {
  ...
  public static <N> Traverser<N> forTree(SuccessorsFunction<N> tree) {
    if (tree instanceof BaseGraph<?> graph) {
      checkArgument(graph.isDirected(), "Undirected graphs can never be trees.");
    }
    if (tree instanceof Network<?,?> network) {
      checkArgument(network.isDirected(), "Undirected networks can never be trees.");
    }
